...
function appendMessage(role, text) {
  ...
  messagesContainer.insertAdjacentHTML("beforeend", `
    <div id="${id}" class="message ${role}">
      ...
      <div class="message-content markdown-body">${processMarkdown(g.parse(text))}</div>
    </div>
  `);
  scrollToBottom();
}
  ...
  switch (msg.type) {
    ...
    case "streamDelta": {
      ...
      if (now - lastRenderTime > RENDER_INTERVAL_MS && currentStreamingMessageId) {
        ...
        if (el) {
          el.innerHTML = processMarkdown(g.parse(streamingBuffer));
          scrollToBottom();
        }
    ...
    case "responseComplete": {
      if (currentStreamingMessageId) {
        const el = document.getElementById(`${currentStreamingMessageId}-content`);
        if (el) {
          el.innerHTML = processMarkdown(g.parse(msg.response?.response_text || streamingBuffer));
          scrollToBottom();
        }
        const badge = document.querySelector(`#${currentStreamingMessageId} .status-badge`);
