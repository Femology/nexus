...
var configuredProviders = /* @__PURE__ */ new Set();
var streamingBuffer = "";
var currentStreamingMessageId = null;
var lastRenderTime = 0;
var RENDER_INTERVAL_MS = 50;
...
function appendMessage(role, text) {
  const id = "msg-" + Date.now();
  const roleName = role === "user" ? "\u{1F464} You" : "\u26A1 Nexus-Code";
  messagesContainer.insertAdjacentHTML("beforeend", `
    <div id="${id}" class="message ${role}">
      <div class="message-role">${roleName}</div>
      <div class="message-content markdown-body">${processMarkdown(marked.parse(text))}</div>
    </div>
  `);
  scrollToBottom();
}
    ...
    wrapper.innerHTML = `
      <div class="code-header">
        ...
        <div class="code-actions">
          <button class="code-action-btn" onclick="navigator.clipboard.writeText(decodeURIComponent('${safe}')).then(()=>{this.textContent='\u2713 Copied';setTimeout(()=>{this.textContent='\u{1F4CB} Copy'},1800)})">\u{1F4CB} Copy</button>
...
function updateProviderCard(provider, isConfigured) {
  const statusEl = document.getElementById(`status-${provider}`);
  const deleteBtn = document.getElementById(`delete-${provider}`);
  const input = document.getElementById(`key-${provider}`);
  if (!statusEl) return;
  if (isConfigured) {
...
function rebuildModelSelector(providers) {
  ...
  providers.forEach((p) => {
    ...
    catalogue.models.forEach((m2) => {
      const opt = document.createElement("option");
      opt.value = m2.value;
      opt.textContent = m2.label;
  ...
  switch (msg.type) {
    ...
    case "streamDelta": {
      ...
      if (now - lastRenderTime > RENDER_INTERVAL_MS && currentStreamingMessageId) {
        ...
        if (el) {
          el.innerHTML = processMarkdown(marked.parse(streamingBuffer));
          scrollToBottom();
        }
    ...
    case "responseComplete": {
      if (currentStreamingMessageId) {
        const el = document.getElementById(`${currentStreamingMessageId}-content`);
        if (el) {
          el.innerHTML = processMarkdown(marked.parse(msg.response?.response_text || streamingBuffer));
          scrollToBottom();
        }
        const badge = document.querySelector(`#${currentStreamingMessageId} .status-badge`);
    ...
    case "toolExecution": {
      ...
      if (containerEl) {
        const rows = msg.tools.map((t) => {
          const icon = t.status === "completed" ? "\u2705" : t.status === "error" ? "\u274C" : "\u{1F504}";
